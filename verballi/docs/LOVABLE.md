...
React + Vite app that runs in the browser, which is exactly what `verballi-wasm`
...
The drop-in kit lives in `crates/verballi-wasm/web/`:
`capture-worklet.js`, `verballi-agent.ts`, `useVerballiAgent.ts`.
...
# → crates/verballi-wasm/pkg/  (verballi_wasm.js + verballi_wasm_bg.wasm + .d.ts)
```
Two ways to consume it in Lovable:
- **Publish to npm** (recommended): `cd pkg && npm publish --access public`, then
  in Lovable `npm i @your-scope/verballi-wasm` and import from it.
- **Colocate**: copy `pkg/` into your app's `src/` and import from `./pkg/...`.
  Put `verballi_wasm_bg.wasm` where Vite serves assets.
...
### 3. Add the kit files
Copy `verballi-agent.ts` and `useVerballiAgent.ts` into `src/`. In
`useVerballiAgent.ts`, fix the import to your package/path:
```ts
import init, { WasmEngine } from "@your-scope/verballi-wasm"; // or "./pkg/verballi_wasm.js"
```

### 4. Use it in a component
```tsx
import { useVerballiAgent } from "./useVerballiAgent";
...
// next session: pass `profile` (decoded) into useVerballiAgent
```
...
- **Echo:** `getUserMedia` requests `echoCancellation: true` and disables the
  browser's own noise suppression/AGC so verballi is the only thing cleaning the
