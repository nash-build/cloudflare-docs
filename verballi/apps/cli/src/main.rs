...
fn cmd_process(args: &[String]) -> Result<(), String> {
    ...
    wav::write_mono_f32(output, &clean, verballi::OUTPUT_SAMPLE_RATE)?;
    println!(
...
fn usage() -> ExitCode {
    eprintln!(
        "verballi {}\n\nUSAGE:\n  verballi enroll  <in.wav> <profile.bin>\n  verballi process <in.wav> <out.wav> [--profile f] [--denoise x] [--focus x] [--proximity x] [--deepfilter x]\n  verballi bench   <in.wav>\n  verballi live    --agent-id <id> [--profile f] [--input-rate 48000]   (build with --features live)",
        verballi::VERSION
    );
    ExitCode::from(2)
}
