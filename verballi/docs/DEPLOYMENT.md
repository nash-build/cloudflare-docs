...
The engine is one Rust crate (`verballi`) exposed through a C ABI
(`verballi-ffi`). Each platform links the compiled library and provides the mic
...
2. Add `crates/verballi-ffi/include/verballi.h` to your bridging header.
...
```bash
./scripts/build-android.sh       # → dist/android/jniLibs/<abi>/libverballi_ffi.so
```
...
```kotlin
companion object { init { System.loadLibrary("verballi_ffi") } }
...
> `verballi-ffi` — it generates idiomatic bindings from the Rust API and removes
