...
## 1. DeepFilterNet → the `verballi-deepfilter` crate
...
This is shipped as the `verballi-deepfilter` crate, built on the official open
...
use verballi_deepfilter::ErbEnhancer;
...
Or from the CLI: `verballi process in.wav out.wav --deepfilter 0.9 --denoise 0.2`.
...
```rust
use verballi_deepfilter::{BandGainModel, ErbEnhancer};
...
```rust
use verballi::speaker::verify::{PresenceScorer, SpeakerVerifier};
...
### Running ECAPA ONNX with the `verballi-onnx` crate

This is shipped — `verballi-onnx` provides `OnnxEcapaVerifier` (a
...
```toml
# Cargo.toml
verballi-onnx = { path = "../crates/verballi-onnx", features = ["ort"] }
```
...
`verballi_onnx::enhance::OnnxEnhancer::from_file(path, frame_size)` at the
`Enhancer` seam.
...
The DSP path runs ~0.001× real time (`verballi bench`). Typical added cost:
