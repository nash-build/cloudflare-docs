#!/usr/bin/env bash
# Build verballi shared libs (.so) for Android ABIs using cargo-ndk.
...
cargo ndk \
  ...
  -o "$OUT" \
  build --release -p verballi-ffi

echo "done: $OUT"
echo "Copy jniLibs/ into your app module (src/main/jniLibs) and load with"
echo "System.loadLibrary(\"verballi_ffi\"); declare the externs via JNI."
