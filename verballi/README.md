...
```
crates/verballi            the engine (DSP + speaker extraction), no I/O, fully tested
crates/verballi-ffi        C ABI (+ header) native shells link against
crates/verballi-deepfilter DeepFilterNet ERB enhancer (open deep_filter crate)
crates/verballi-onnx       ONNX adapters: ECAPA verifier, ONNX enhancer, log-mel front-end
crates/verballi-wasm       WebAssembly bindings + web kit (browser / Electron / Lovable)
...
./target/release/verballi enroll you.wav you.profile

# 2) Isolate a file
./target/release/verballi process noisy.wav clean.wav --profile you.profile
...
./target/release/verballi live --agent-id <AGENT_ID> --profile you.profile
```

## Using the engine from Rust

```rust
use verballi::{Config, VoiceEngine};
...
`verballi bench`), leaving ample budget to drop in a neural denoiser
...
|------------|------|------------|--------|
| Neural denoise | `verballi-deepfilter` (`Enhancer`) | **DeepFilterNet** (Rust, MIT/Apache) | ERB enhancer runnable now; trained-weights slot via `BandGainModel` |
| Per-bin neural mask | `dsp::denoise::NeuralDenoiser` | DTLN / custom | seam ready |
| Window-level speaker lock | `speaker::verify::PresenceScorer` + `SpeakerVerifier` | **ECAPA-TDNN** (ONNX) | rolling re-score runnable now (MFCC); ECAPA adapter shipped in `verballi-onnx` |
| ONNX runtime adapters | `verballi-onnx` (`OnnxEcapaVerifier`, `OnnxEnhancer`, `MelFrontend`) | any ONNX export | compiled against `ort`; dynamic-loaded runtime, bring a model |

```bash
# DeepFilterNet ERB enhancer through the CLI:
verballi process noisy.wav clean.wav --deepfilter 0.9 --denoise 0.2
```
...
[`docs/LOVABLE.md`](docs/LOVABLE.md) (`crates/verballi-wasm/web/`).
